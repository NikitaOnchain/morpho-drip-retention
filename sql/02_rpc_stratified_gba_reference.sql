...
Business question
  ...
  Blockchain Analytics population for a selected stratified Phase 2 pilot day?

Population
  Official Morpho Blue contract on Arbitrum, eight critical event families,
  45 DRIP-eligible market IDs, and one complete UTC day supplied by the runner.
...
Cost checkpoint
  Run a dry run for each substituted UTC window before execution. The sum of
  the two new stratified windows must remain within the 8-10 GiB pilot gate.
  2025-07-09 dry run: 5,587,488,684 bytes; actual: 5,410,352,105 bytes.
  2026-02-18 dry run: 4,037,319,193 bytes; actual: 3,860,182,614 bytes.
  Combined dry-run upper bound: 9,624,807,877 bytes (8.964 GiB).

Important limitation
  This is a parameterized one-day pilot reference, not a full-window
  extraction. The runner replaces __START_UTC__ and __END_EXCLUSIVE_UTC__.
  Unrelated Morpho event families and non-eligible market IDs are excluded.
*/
...
WHERE block_timestamp >= TIMESTAMP('__START_UTC__')
  AND block_timestamp < TIMESTAMP('__END_EXCLUSIVE_UTC__')
